...
| 触发条件 | 一线修复 | 仍失败兜底 |
|----------|----------|------------|
| `marker`/`MinerU` 安装失败 | 换用 `pypdf` + 正则提取文本 | 人工对照 PDF 重建题干，并记录该文件需后续补 OCR |
| 提取结果乱码/缺题 | 调整提取参数或换另一工具重试 | 人工分段复制原文到 JSON，低置信度题目标 `[待核对]` |
| 源 JSON 已存在且含人工答案 | 只更新题干/选项/配图字段 | 用户明确回复「覆盖」前禁止动 `answer`/`solution` |
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|----------|----------|------------|
| 多模型答案不一致 | 引入第 3 种独立来源仲裁 | 标记 `[待核对]`，交付时告知用户 |
| `sympy` 求解超时或无法化简 | 换数值代入多个特例验证 | 标记 `[待核对]` |
| 用户答案格式不统一 | 用第 4 章 `normalizeAnswer()` 归一化 | 多空/多问用 `;` 分隔后仍异常则向用户确认 |
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|----------|----------|------------|
| `$...$` 花括号不闭合 | 回到源 JSON 补全括号 | 公式含复杂嵌套时先转 `$$...$$` 并人工复核 |
| MathJax 渲染报错 | 检查并移除非法命令或转义 | 仍报错则截图或复制报错信息交付时说明 |
| 图片路径失效 | 确认 `assets/` 与 HTML 相对位置 | 无法修复则删除 `image` 字段或提示用户原卷查看 |
