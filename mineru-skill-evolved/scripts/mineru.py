...
# Standard-API failures that are worth falling back to the Agent API (when input fits).
STANDARD_FALLBACK_CODES = {
    "A0202", "A0211",            # invalid / expired token
    -60017, -60018, -60019,      # quota / retry-limit exhausted
    429,                         # rate limited
    -10001, -60001, -60007, -60009, -60010,  # service / queue / parse transient
}

# Agent API supports everything except HTML.
AGENT_SUPPORTED_SUFFIXES = SUPPORTED_SUFFIXES - {".html"}
...
def choose_api(
    *,
    ...
    if not token:
        return "agent"
    # With a token, prefer Standard API for accuracy / batch / extra formats.
    if batch or extra_formats:
        return "standard"
    if size_bytes is not None and size_bytes > STANDARD_MAX_BYTES:
        return "standard"
    return "standard"
...
# --------------------------------------------------------------------------- #
# Token acquisition & smart routing helpers
# --------------------------------------------------------------------------- #
_token_valid_cache: dict = {}
...
def _prompt_token(reason: str) -> Optional[str]:
    if not _is_interactive():
        return None
    print(f"{reason}", file=sys.stderr)
    print("Get a token at https://mineru.net/apiManage/token", file=sys.stderr)
    try:
        return getpass.getpass("Paste token (input hidden): ").strip() or None
    except (EOFError, OSError):
        return None


def _validate_token(token: Optional[str]) -> bool:
    """Lightweight probe; invalid only on auth errors A0202/A0211."""
    if not token:
        return False
    cached = _token_valid_cache.get(token)
    if cached is not None:
        return cached
    try:
        # A valid token will be accepted at the gateway but fail on missing params.
        _api_json("POST", f"{STANDARD_API}/extract/task", token=token, payload={})
        _token_valid_cache[token] = True
    except MinerUError as exc:
        if exc.code in ("A0202", "A0211"):
            _token_valid_cache[token] = False
        else:
            _token_valid_cache[token] = True
    except Exception:
        # Network/service issues: do not poison cache; assume valid.
        return True
    return _token_valid_cache[token]


def _acquire_token(*, reason: str, allow_prompt: bool = True) -> Optional[str]:
    """Return a usable token from env/args or an interactive prompt."""
    token = os.environ.get("MINERU_TOKEN")
    if token and _validate_token(token):
        return token
    if token and not _validate_token(token):
        print("Warning: existing MINERU_TOKEN appears invalid; ignoring it.", file=sys.stderr)
        token = None
    if not token and allow_prompt:
        token = _prompt_token(reason)
        if token and _validate_token(token):
            os.environ["MINERU_TOKEN"] = token
            return token
        if token:
            print("Warning: provided token appears invalid.", file=sys.stderr)
            token = None
    return token


def _needs_standard(api: str, sources: list, opts: ParseOptions) -> bool:
    """Whether this run cannot be handled solely by the Agent API."""
    if api == "standard":
        return True
    if api != "auto":
        return False
    ...
    for src in sources:
        if is_html(src):
            return True
        if suffix_of(src) not in AGENT_SUPPORTED_SUFFIXES:
            return True
        if is_url(src):
            continue
        try:
            ...
            size = 0
        if size <= 0 or size > AGENT_MAX_BYTES:
            return True
        if suffix_of(src) == ".pdf":
            pages = _pdf_page_count_if_available(src)
            if pages is not None and pages > AGENT_MAX_PAGES:
                return True
    return False


def _agent_can_handle(source: str, opts: ParseOptions) -> bool:
    """Check whether a specific input is within Agent API limits."""
    if opts.extra_formats:
        return False
    if is_html(source):
        return False
    if suffix_of(source) not in AGENT_SUPPORTED_SUFFIXES:
        return False
...
def _standard_failure_is_fallback(exc: MinerUError) -> bool:
    return exc.code in STANDARD_FALLBACK_CODES
...
def process_one(
    ...
    *,
    ...
    try:
        ...
        if chosen == "standard":
            if not token:
                token = _acquire_token(reason="Standard API requires a token for this input")
            if not token:
                raise MinerUError(
            ...
            try:
                ...
                # Smart fallback: if Standard fails for a recoverable reason and the
                # input fits Agent limits, try the free Agent API.
                if api == "auto" and _standard_failure_is_fallback(exc) and _agent_can_handle(source, opts):
                    result.api = "agent"
...
def _ensure_token_for_standard_jobs(jobs, api, opts) -> Optional[str]:
    """If any pending job needs Standard API, make sure we have a valid token."""
    if api == "agent":
        return os.environ.get("MINERU_TOKEN")
    pending_std = [j for j in jobs if not j.finished and j.api == "standard"]
    if not pending_std:
        return os.environ.get("MINERU_TOKEN")
    return _acquire_token(
        reason="Some inputs require the Standard API (HTML, extra formats, or size/pages exceed Agent limits)"
    )


def _reserve_agent_single(job, opts, timeout) -> list:
    """Submit one job via the Agent API; return upload work ``[(job, url)]`` (empty for URLs)."""
    src = job.source
...
def _reserve_single(job, opts, token, timeout) -> list:
    ...
    if job.api == "agent":
        return _reserve_agent_single(job, opts, timeout)
    src = job.source
    job.deadline = time.monotonic() + timeout
    req_timeout = min(timeout, REQUEST_TIMEOUT_CAP)
...
def _poll_until_done(active, opts, token, output_dir, *, poll_interval, obsidian,
                     want_markdown, download_pool, on_done, timeout=60, api="auto"):
    ...
    while groups:
        ...
        for poll_id in due_ids:
            ...
            try:
                ...
                completed = []
                if api == "auto" and _standard_failure_is_fallback(exc) and st["kind"] != "agent":
                    failed = []
                    fallback_jobs = []
                    for job in list(group):
                        if _agent_can_handle(job.source, opts):
                            try:
                                _reserve_agent_single(job, opts, timeout)
                                fallback_jobs.append(job)
                                group.remove(job)
                            except Exception as agent_exc:
                                job.result.error = (
                                    f"Standard poll failed: {exc}; "
                                    f"Agent fallback failed: {agent_exc}"
                                )
                                job.result.state = STATE_FAILED
                                failed.append(job)
                                group.remove(job)
                    for job in fallback_jobs:
                        new_st = groups.setdefault(
                            job.poll_id,
                            {"kind": "agent", "jobs": [], "interval": poll_interval, "due": now}
                        )
                        new_st["jobs"].append(job)
                elif _is_retryable_api_error(exc):
...
def run_pipeline(sources, opts, *, token, output_dir, api, resume, poll_interval,
    ...
    # Acquire a token if any pending job needs Standard API.
    token = _ensure_token_for_standard_jobs(pending, api, opts)
    ...
    with ThreadPoolExecutor(max_workers=workers) as submit_pool, \
        ...
        def _try_agent_fallback_for_jobs(jobs, original_exc):
            for job in jobs:
                if job.finished or job.api != "standard":
                    continue
                if not _agent_can_handle(job.source, opts):
                    job.result.state = STATE_FAILED
                    job.result.error = f"submit failed: {original_exc}"
                    job.finished = True
                    _notify(job)
                    continue
                try:
                    job.api = "agent"
                    job.result.api = "agent"
                    uploads.extend(_reserve_agent_single(job, opts, timeout))
                except Exception as agent_exc:
                    agent_msg = str(agent_exc) if isinstance(agent_exc, MinerUError) else f"{type(agent_exc).__name__}: {agent_exc}"
                    job.result.state = STATE_FAILED
                    job.result.error = f"Standard failed: {original_exc}; Agent fallback failed: {agent_msg}"
                    job.finished = True
                    _notify(job)
        ...
        for future in as_completed(reserve):
            ...
            try:
                ...
                if api == "auto" and _standard_failure_is_fallback(exc):
                    _try_agent_fallback_for_jobs(affected, exc)
                else:
                    msg = str(exc)
                    for job in affected:
...
def main(argv=None) -> int:
    ...
    # Smart token handling: when Standard API is unavoidable, validate the token
    # or interactively prompt for one. Otherwise the skill falls back to Agent.
    if _needs_standard(args.api, sources, opts):
        if token and not _validate_token(token):
            _log("Warning: provided MINERU_TOKEN appears invalid; ignoring it.", quiet=args.quiet)
            token = None
        if not token:
            token = _acquire_token(reason="Standard API is required for this input")
        if not token:
            _log("Standard API required but no valid token provided. "
                 "Set MINERU_TOKEN or pass --token.", quiet=args.quiet)
            return 1
    ...
    token_label = "token set" if token else "no token (Agent API)"
    _log(
        f"📚 {len(sources)} input(s) · workers={workers} · {token_label}",
        quiet=args.quiet,
    )
